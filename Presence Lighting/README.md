...
| **Check timer** | Blocked if `timer.briefly_minimize_upstairs_lighting` is `active` |
| **Check mode** | Blocked if `input_boolean.minimize_upstairs_lighting` is `on` |
| **Check both** | Blocked if either the timer is active or the input_boolean is on |
